...
namespace WinformsVisualization.Visualization
{
    public class SpectrumBase : INotifyPropertyChanged
    {
        ...
        protected const double MinDbValue = -70;
        protected const double MaxDbValue = 20;
        ...
        List<float> aveDataPoints = new List<float>();
        protected virtual SpectrumPointData[] CalculateSpectrumPoints(double maxValue, float[] fftBuffer)
        {
            ...
            for (int i = _minimumFrequencyIndex; i <= _maximumFrequencyIndex; i++)
            {
                           ...
                           : _spectrumIndexMax[spectrumPointIndex]))
                {
                    ...
                    if (spectrumPointIndex < aveDataPoints.Count)
                        aveDataPoints[spectrumPointIndex] = (float)(aveDataPoints[spectrumPointIndex] * .3f + value * .7f);
                    else
                        aveDataPoints.Add((float)value);
                    value = aveDataPoints[spectrumPointIndex];
